...
export default function MedidaAdminRow({ medida }) {
    ...
    const handleUpdate = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        updateForm.post(route('admin.medidas.update'), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const deleteMarca = () => {
        if (confirm('¿Estas seguro de eliminar esta medida?')) {
            updateForm.delete(route('admin.medidas.destroy'), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    return (
        <tr className={`border text-black odd:bg-gray-100 even:bg-white`}>
            <td className="h-[90px] align-middle">{medida?.order}</td>
            <td className="align-middle">{medida?.name}</td>
                    ...
                    >
                        <form onSubmit={handleUpdate} method="POST" className="text-black">
                            <div className="w-[500px] rounded-md bg-white p-4">
                                <h2 className="mb-4 text-2xl font-semibold">Actualizar Medida</h2>
